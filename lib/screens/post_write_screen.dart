...
class _PostWriteScreenState extends State<PostWriteScreen> {
  ...
  // 2. 게시글 작성 완료 처리 (Firebase 연동 핵심 로직)
  Future<void> _handleSubmit() async {
    ...
    try {
      final String itemId = FirebaseFirestore.instance.collection('items').doc().id;
      ...
      final newItem = ItemModel(
        id: itemId,
        userId: widget.userId, // 💡 판매자(userId) 자동 등록
      ...
      _showSnackbar('게시글 등록이 완료되었습니다!', success: true);
      if (mounted) {
      ...
      print('게시글 등록 오류: $e');
      _showSnackbar('게시글 등록 중 오류가 발생했습니다: $e', success: false);
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('내 물건 팔기', style: TextStyle(color: Colors.black)),
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('${_selectedImages.length}/10', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                ],
              ),
            ),
          ),
